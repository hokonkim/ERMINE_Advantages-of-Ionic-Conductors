...
registerMooseObject("ermineApp", InterfaceChargeTransferFullyCoupled);
...
InterfaceChargeTransferFullyCoupled::InterfaceChargeTransferFullyCoupled(const InputParameters & parameters) :
...
Real
InterfaceChargeTransferFullyCoupled::computeQpResidual(Moose::DGResidualType type)
{
  Real b    = _z * _F / _R / _T;
  ...
  Real eta_ct = E_conc - (_phi_LSM - _neighbor_value[_qp]);
...
Real
InterfaceChargeTransferFullyCoupled::computeQpJacobian(Moose::DGJacobianType type)
{
  Real b    = _z * _F / _R / _T;
  Real logV = log10(_u[_qp]);
  Real logP = -2.173913*logV - 17.173913;
