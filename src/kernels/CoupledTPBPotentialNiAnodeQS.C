...
registerMooseObject("ermineApp", CoupledTPBPotentialNiAnodeQS);
...
InputParameters validParams<CoupledTPBPotentialNiAnodeQS>()
{
  ...
  params.addClassDescription("Fully Coupled TPB reaction source kernel in Butler-Volmer sinh() form for potential in Ni. j = 2 * j0 * sinh(coef * eta)");
...
CoupledTPBPotentialNiAnodeQS::CoupledTPBPotentialNiAnodeQS(const InputParameters & parameters) :
    Kernel(parameters),
...
Real
CoupledTPBPotentialNiAnodeQS::computeQpResidual()
{
  Real b       = _z * _F / _R / _T;
  ...
  Real eta     = E_conc - (_phi_YSZ[_qp] - _u[_qp]);
...
Real
CoupledTPBPotentialNiAnodeQS::computeQpJacobian()
{
  Real b       = _z * _F / _R / _T;
  Real E_conc  = _R * _T / 4 / _F * log(_pO2_CE[_qp] / (pow (_p_H2O[_qp]/((_p_total-_p_H2O[_qp]) * _Kf), 2.0)));
  // Real phi_Ni  = _func_phi_Ni.value(_t, _q_point[_qp]);
...
Real
CoupledTPBPotentialNiAnodeQS::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _num_p_H2O)
  {
    Real b       = _z * _F / _R / _T;
